...
def LSTM_model(conf_lstm, model):
    for ind, layers in enumerate(conf_lstm):
        ...
        model.add(Bidirectional(LSTM(25, dropout=0.2, return_sequences=return_sequences, activation="tanh")))
        if return_sequences:
...
def NN_model(conf_cnn_lstm, conf_mlp, classifier, num_class, part_name):
    ...
    # add input layer
    if part_name == 'All':
        model.add(Input(shape=(None, 5, 11)))  # 11, 9
        # reshape into 3D (feature and parts together LLDsx5)
        model.add(Reshape((-1, 55)))  # 55, 45
    else:
        model.add(Input(shape=(None, 11)))  # 11, 9
...
def perform_CNN_LSTM(X_train, Y_train, X_dev, Y_dev, X_test, Y_test, num_class, N_samples_fold, classifier, part_name):
    results                 = []
    configurations_mlp      = ((75, 75),)
...
def feature_normalisation(dic_train_x, dic_dev_x, dic_test_x, dic_train_y, dic_dev_y, dic_test_y, part_name):
    ...
    reshaped_train = scaler.fit_transform(X_train.reshape(-1, 11))  # transform train: 11, 9
    X_train        = reshaped_train.reshape(X_train.shape)  # reshape back the normalised features into 4D
    reshaped_dev   = scaler.transform(X_dev.reshape(-1, 11))  # transform dev: 11, 9
    X_dev          = reshaped_dev.reshape(X_dev.shape)
    reshaped_test  = scaler.transform(X_test.reshape(-1, 11))  # transform test: 11, 9
    X_test         = reshaped_test.reshape(X_test.shape)

    return X_train, Y_train, X_dev, Y_dev, X_test, Y_test


def run_experiments(ALL_dics, folds, classifier, num_class, part_name):
    ...
    # RUN CLASSIFIER
    predictions, best_Conf, Y_test = perform_CNN_LSTM(X_train, Y_train, X_dev, Y_dev, X_test, Y_test, num_class, N_samples_fold, classifier, part_name)
...
def run_main_function(results, dic_features, num_class, classifier, all_results, part_name, A, B, C):
    ...
    for index, folds in enumerate(all_folds):
        ...
        predictions, best_Conf, Y_test = run_experiments(ALL_dics, folds, classifier, num_class, part_name)
...
def experiments_per_part(splits, num_class, dic_features, all_results, my_dir, classifier, part_name, num_feat):
    for split in splits:
        ...
        results = []
        results, all_results = run_main_function(results, dic_features, num_class, classifier, all_results, part_name, A, B, C)
