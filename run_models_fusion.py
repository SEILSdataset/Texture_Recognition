...
from tensorflow.keras.layers import Dense, Input, Dropout, Conv1D, MaxPool1D, GlobalMaxPool1D, BatchNormalization, Activation, LSTM, Bidirectional, Reshape, Flatten, Concatenate
...
def LSTM_model(conf_lstm, net):
    for ind, layers in enumerate(conf_lstm):
        ...
        net = Bidirectional(LSTM(25, dropout=0.2, return_sequences=return_sequences, activation="tanh"))(net)
        if return_sequences:
...
def CNN_model(conf_cnn, net):
    for layers in conf_cnn:
        net = Conv1D(625, (3,), strides=2, activation="linear", padding='same')(net)
        net = BatchNormalization()(net)
...
def NN_model(conf_cnn_lstm, conf_mlp, classifier, num_class, part_name):
    # add input layer
    if part_name == 'All':
        input_llds = Input(shape=(None, 5, 9))  # 11, 9
        # reshape into 3D (feature and parts togethere 10x5)
        net = Reshape((-1, 45))(input_llds)  # 55, 45
    else:
        input_llds = Input(shape=(None, 9))  # 11, 9
        net = input_llds
    ...
    for neurons in conf_mlp:
        ...
        if counter < len(conf_mlp):
            # dropout 20%, i.e. 20% of the weights of each hidden layer (except the last) are set to 0 during training
            net = Dropout(0.3)(net)
...
def perform_CNN_LSTM(X_train, Y_train, X_dev, Y_dev, X_test, Y_test, num_class, N_samples_fold, classifier, part_name, dic_train_MLP, dic_dev_MLP, dic_test_MLP):
    results                 = []
    batch_sizes             = (35, )
    configurations_mlp      = ((20, 20),)
    learning_rates          = (0.001, )
    configurations_cnn_lstm = [[1, ], ]
    best_UAR                = 0.
        ...
        for conf_mlp in configurations_mlp:
            for learn in learning_rates:
                for batch_size in batch_sizes:
                    ...
                    # build the model
                    model     = NN_model(conf_cnn_lstm, conf_mlp, classifier, num_class, part_name)
...
def feature_normalisation(dic_train_x, dic_dev_x, dic_test_x, dic_train_y, dic_dev_y, dic_test_y, part_name):
    ...
    reshaped_train = scaler.fit_transform(X_train.reshape(-1, 9))  # reshape to 2D and apply feature normalisation (-1 means that the first three dim are stacked together): 11, 9
    X_train        = reshaped_train.reshape(X_train.shape)  # reshape back the normalised features into 4D

    # transform dev
    reshaped_dev = scaler.transform(X_dev.reshape(-1, 9))  # 11, 9
    X_dev        = reshaped_dev.reshape(X_dev.shape)

    # transform test
    reshaped_test = scaler.transform(X_test.reshape(-1, 9))  # 11, 9
    X_test        = reshaped_test.reshape(X_test.shape)

    return X_train, Y_train, X_dev, Y_dev, X_test, Y_test


def run_experiments(ALL_dics, folds, classifier, num_class, part_name, dic_train_MLP, dic_dev_MLP, dic_test_MLP):
    # FEATURE NORMALISATION
    new_dict = copy.deepcopy(ALL_dics)
    X_train, Y_train, X_dev, Y_dev, X_test, Y_test = feature_normalisation(new_dict['X' + folds[0]], new_dict['X' + folds[1]], new_dict['X' + folds[2]], new_dict['Y' + folds[0]], new_dict['Y' + folds[1]], new_dict['Y' + folds[2]], part_name)
    ...
    # RUN CLASSIFIER
    predictions, best_Conf, Y_test = perform_CNN_LSTM(X_train, Y_train, X_dev, Y_dev, X_test, Y_test, num_class, N_samples_fold, classifier, part_name, dic_train_MLP, dic_dev_MLP, dic_test_MLP)
    return predictions, best_Conf, Y_test
...
def run_main_function(results, dic_features, num_class, classifier, all_results, part_name, A, B, C, ALL_dics_MLP, N_functionals):
    ...
    for index, folds in enumerate(all_folds):
        ...
        predictions, best_Conf, Y_test = run_experiments(ALL_dics, folds, classifier, num_class, part_name, dic_train_MLP, dic_dev_MLP, dic_test_MLP)
...
def experiments_per_part(splits, num_class, dic_features, all_results, my_dir, classifier, part_name, num_feat, features_functionals):
    for split in splits:
        ...
        results, all_results = run_main_function(results, dic_features, num_class, classifier, all_results, part_name, A, B, C, ALL_dics_MLP, N_functionals)
...
def set_up(classifier, num_class, num_feat):
    ...
    features_functionals = pd.read_csv(my_dir + '/functionals_Deltas_all.csv', sep='\t')
    features, features_C, features_A, features_Q, features_T, features_B = get_features(my_dir)
...
if __name__ == '__main__':
    """
    num_class = 3 (ANT, CON, HOM); 2 (CON, HOM)
    """
    classifier = 'CNN'
    num_class  = 2  # 2, 3
    num_feat   = 9
